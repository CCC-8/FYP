...
<body>
    ...
    <section id="editProduct" class="contact mt-5 mb-5">
        <div class="container">
            ...
            <form action="/UpdateProduct/{{ $product->id }}" method="post" class="php-email-form mt-4" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-lg-4 mb-3" data-aos="fade-right">
                        <img src="{{ asset('images/' . $product->image) }}" class="img-fluid" id="productPreviewImage"
                            style="object-fit: contain; width: 270px; height: 270px; transform: translate3d(0, 0, 1px);">
                        <h6 class="pt-3">Upload a different product image...</h6>
                ...
                <div class="row">
                    <div class="col-md-4 form-group mt-3 mt-md-0">
                        <input type="text" name="name" class="form-control" id="name"
                            placeholder="Product Name / Model" value="{{ $product->name }}">
                    </div>
                    <div class="col-md-4 form-group mt-3 mt-md-0">
                        <select class="form-control" name="type"
                            ...
                            <option value="Vehicle" {{ $product->type === 'Vehicle' ? 'selected' : '' }}>Vehicle
                            </option>
                            <option value="Tyre" {{ $product->type === 'Tyre' ? 'selected' : '' }}>Tyre</option>
                ...
                <div class="row mt-3">
                    <div class="col-md-6 form-group">
                        <select class="form-control" name="condition"
                            ...
                            <option value="Defected" {{ $product->condition === 'Defected' ? 'selected' : '' }}>Defected
                            </option>
                        </select>
                    </div>
                    <div class="col-md-6 form-group">
                        <select class="form-control" name="status"
                            ...
                            <option value="Reserved" {{ $product->status === 'Reserved' ? 'selected' : '' }}>Reserved
                            </option>
                            <option value="Received" {{ $product->status === 'Received' ? 'selected' : '' }}>Received
                            </option>
                            <option value="Returned" {{ $product->status === 'Returned' ? 'selected' : '' }}>Returned
                            </option>
                        </select>
                    </div>
                </div>
                <div class="form-group mt-3">
                    <textarea class="form-control" name="description" rows="3" placeholder="Product Description">{{ $product->description }}</textarea>
                </div>
                <div class="my-3">
